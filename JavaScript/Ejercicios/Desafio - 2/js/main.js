...
class Biblioteca {
    ...
    mostrarBiblioteca = () => {
        for(let i of this.arrayLibros){
            console.log(i.mostrarLibros());
        }
    }
    comprarLibro = () => {
    ...
    mostrarLibrosComprados = (arrayLibros) => {
        let cadena = "[USTED ADQUIRIÓ]\n";
        for(let i of arrayLibros){
            cadena += `${i.mostrarLibros()}\n`
        }
        cadena += `Total sin iva: $${this.calcularPrecioTotal(arrayLibros)}\nTotal con 21% iva: $${this.calcularPrecioTotalMasIva(arrayLibros)}`
        return cadena;
    }
}
...
if(cantidad != -1){
    while(i < cantidad){
        ...
        if(libro != -1)
            ...
            mostrar = false;
            console.log('Se cancelo la operación ya que introdujo un legajo invalido');
            alert('Operación iterrumpida');
            break;
        }
        i++;
    }
