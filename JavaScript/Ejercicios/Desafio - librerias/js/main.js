...
class Biblioteca {
    ...
    verificarStock = (libro) =>{
        ...
        for(let i = 0; i < biblio.length; i++){
            if(biblio[i].legajo == libro.legajo){
                if(biblio[i].stock > 0)
                    return true;
            }
        }
...
biblio.arrayLibros.forEach(i => {
    ...
    libro.addEventListener('click', (e) =>{
        ...
        if(index != false){                                                                                 
                biblio.verificarStock(lib) ? (indexStock && biblio.librosSeleccionados(biblio.verificarRepetidos(acumulador, lib))) : biblio.error(2);
            }else
                biblio.error(0);
        }else
            biblio.error(1);
